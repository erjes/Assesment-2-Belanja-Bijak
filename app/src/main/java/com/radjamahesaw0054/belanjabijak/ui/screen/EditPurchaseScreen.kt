...
fun EditPurchaseScreen(
    ...
    val itemExisting = remember(pengeluaranList, idPurchase) {
        pengeluaranList.find { it.id == idPurchase }
    }

    var namaBarang by remember { mutableStateOf("") }
    var harga by remember { mutableStateOf("") }
    var jumlah by remember { mutableStateOf("") }
    var tanggalByItem by remember { mutableStateOf("") }
    ...
    LaunchedEffect(itemExisting) {
        itemExisting?.let {
            namaBarang = it.namaBarang
            harga = it.harga.toInt().toString()
            jumlah = it.jumlah.toString()
            tanggalByItem = it.tanggal
            selectedKategoriKey = it.kategori
            val index = kategoriKeys.indexOf(it.kategori)
            if (index != -1) {
                selectedDisplayKategori = kategoriDisplayOptions[index]
            }
        }
    }

    if (openDialogConfirm && itemExisting != null) {
        DisplayAlertDialog(
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(stringResource(R.string.title_edit)) },
                navigationIcon = {
                ...
                actions = {
                    IconButton(onClick = { openDialogConfirm = true }) {
            ...
            Button(
                onClick = {
                    ...
                    if (namaBarang.isBlank() || nominalHarga == null || totalJumlah == null) {
                        ...
                        val isDataSame = itemExisting != null &&
                                namaBarang == itemExisting.namaBarang &&
                                nominalHarga == itemExisting.harga &&
                                totalJumlah == itemExisting.jumlah &&
                                selectedKategoriKey == itemExisting.kategori
                        ...
                        if (isDataSame) {
                            ...
                            viewModel.ubahPengeluaran(
                                id = idPurchase,
                                nama = namaBarang,
                                harga = nominalHarga,
                                jumlah = totalJumlah,
                                kategori = selectedKategoriKey,
                                tanggal = tanggalByItem
                            )
                            navController.popBackStack()
                        }
                    }
